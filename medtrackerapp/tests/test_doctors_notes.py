...
from medtrackerapp.models import DoctorsNote
...
class DoctorsNotesTest(APITestCase):
    ...
    def test_create_not_missing_fields(self):
        data = {
            "content": "Missing medication field."
        }
        response = self.client.post(self.url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_update_note_not_allowed(self):
        ...
        response = self.client.put(url, {"text": "Trying update"})
